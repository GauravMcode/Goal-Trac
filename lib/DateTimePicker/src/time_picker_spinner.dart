...
class _TimePickerSpinnerState extends State<TimePickerSpinner> {
  ...
  void initState() {
    ...
    if (widget.onTimeChange != null) {
      _ambiguate(WidgetsBinding.instance)!.addPostFrameCallback((_) => widget.onTimeChange!(getDateTime()));
    }
  }
  ...
  Widget apSpinner() {
    Widget spinner = NotificationListener<ScrollNotification>(
      ...
      child: ListView.builder(
        itemBuilder: (context, index) {
          String text = index == 1 ? widget.amText ?? 'AM' : (index == 2 ? widget.pmText ?? 'PM' : '');
          return Container(
            height: _getItemHeight(),
